...
${this.compileErrors.map(formatError).join('\n')}`);
  ...
  private async distDirs(): Promise<PathOsBasedRelative[]> {
    const relativeCompDir = this.workspace.componentDir(new ComponentID(this.component.id), undefined, {
      relative: true,
    });
    const packageName = componentIdToPackageName(this.component);
    const packageDir = path.join('node_modules', packageName);
    return [packageDir, ...(await this.dependencyResolver.getDistDirs(this.workspace.path, relativeCompDir))].map(
      (dist) => path.join(dist, DEFAULT_DIST_DIRNAME)
    );
  }
